      ...
      <div className="container flex h-16 items-center">
        <div className="mr-4 flex flex-1">
          <Link href="/" className="mr-6 flex items-center space-x-2">
            <span className="font-bold text-xl text-teal">Parallel Graph Algorithms</span>
          </Link>
            ...
            {routes.map((route) => (
              <Link
                ...
                className={cn(
                  ...
                  pathname === route.path ? "text-teal font-semibold" : "text-foreground/60",
                )}
              >
                {route.name}
              </Link>
        ...
        <Sheet>
          ...
          <SheetContent side="left" className="pr-0">
            <Link href="/" className="flex items-center mb-8">
              <span className="font-bold text-lg text-teal">Parallel Graph Algorithms</span>
            </Link>
            <nav className="flex flex-col gap-4">
              {routes.map((route) => (
                <Link
                  ...
                  href={route.path}
                  className={cn(
                    "text-foreground/60 transition-colors hover:text-teal",
                    pathname === route.path ? "text-teal font-semibold" : "text-foreground/60",
                  )}
                >
                  {route.name}
